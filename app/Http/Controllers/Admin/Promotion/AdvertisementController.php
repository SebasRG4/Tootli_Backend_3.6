...
class AdvertisementController extends Controller
{
    ...
    public function store(AdvertisementStoreRequest $request)
    {
        ...
        if ($advertisement->store_id) {
            $advertisement->module_id = $advertisement->store?->module?->id;
            $advertisement->module_type = $advertisement->store?->module?->module_type;
        } else {
            // Assign current module ID if available in config, otherwise null or default
            $advertisement->module_id = config('module.current_module_data')['id'] ?? config('module.current_module_id');
            $advertisement->module_type = config('module.current_module_data')['module_type'] ?? 'all';
        }
        $advertisement->save();
    ...
    public function copyAddPost(Advertisement $advertisement, AdvertisementUpdateRequest $request)
    {
        ...
        if ($newAdvertisement->store_id) {
            ...
            $newAdvertisement->module_id = config('module.current_module_data')['id'] ?? config('module.current_module_id');
            $newAdvertisement->module_type = config('module.current_module_data')['module_type'] ?? 'all';
        }
        $newAdvertisement->save();
