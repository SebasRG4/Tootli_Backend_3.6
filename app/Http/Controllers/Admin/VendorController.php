    ...
    public function update(Request $request, Store $store)
    {
        ...
        if ($request->removedImageKeys) {
            ...
            foreach ($infrastructure_images as $key => $value) {
                ...
                if (in_array($imgName, $removedKeys)) {
                    Helpers::check_and_delete('store/', $imgName);
        ...
        if ($request->has('infrastructure_images')) {
            foreach ($request->infrastructure_images as $img) {
                $image_name = Helpers::upload('store/', 'png', $img);
                $infrastructure_images[] = ['img' => $image_name, 'storage' => Helpers::getDisk()];
            }
        }
        $store->infrastructure_images = $infrastructure_images;
