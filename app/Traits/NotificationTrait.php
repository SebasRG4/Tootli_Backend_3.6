...
//        }
        ...
        // --- NEW GO WORKER DELEGATION ---
        // Instead of waiting for Google's HTTP response in PHP, 
        // we delegate the massive push campaign to the ultra-fast Go Worker
        $goPayload = [
            'type' => 'push_campaign',
            'data' => $postData
        ];
        ...
        try {
            \Illuminate\Support\Facades\Redis::lpush('tootli:go_jobs', json_encode($goPayload));
            return true;
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error("Failed to enqueue push_campaign to Go: " . $e->getMessage());
            // Fallback to old PHP method if Redis is down
