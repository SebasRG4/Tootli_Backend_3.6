    ...
    <div class="content container-fluid">
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    {{translate('Control de Efectivo y Asignación')}}
                </span>
            </h1>
        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label class="input-label">{{translate('Límite Global de Efectivo en Mano')}}</label>
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label class="input-label">{{translate('Umbral de Alto Ticket ($)')}}</label>
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label class="input-label">{{translate('Tiempo Máximo sin Depósito (minutos)')}}</label>
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label class="input-label">{{translate('Estrategia para Alto Valor')}}</label>
                                <select name="high_value_strategy" class="form-control">
                                    <option value="only_card" {{($high_value_strategy && $high_value_strategy->value == 'only_card') ? 'selected' : ''}}>{{translate('Solo Tarjeta')}}</option>
                                    <option value="relaxed_cash" {{($high_value_strategy && $high_value_strategy->value == 'relaxed_cash') ? 'selected' : ''}}>{{translate('Efectivo Relajado (Asignar aunque exceda)')}}</option>
                                    <option value="assign_any" {{($high_value_strategy && $high_value_strategy->value == 'assign_any') ? 'selected' : ''}}>{{translate('Asignar a cualquiera')}}</option>
                                    <option value="notify_admin" {{($high_value_strategy && $high_value_strategy->value == 'notify_admin') ? 'selected' : ''}}>{{translate('Notificar Admin y Esperar')}}</option>
                                </select>
                            </div>
                        </div>
                    </div>
