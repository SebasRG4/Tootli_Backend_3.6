    ...
    <form id="zone-setup-form" action="{{ route('admin.business-settings.admin-landing-page-settings', 'available-zone-section') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-sm-6">
                        {{ 'Para ver una lista de todas las zonas activas en su' }} <a href="{{route('home')}}" target="_blank" class="text--underline text-006AE5">{{ 'Aterrizaje de administrador' }}</a> {{ 'Página,' }} <br class="d-none d-md-inline-block"> {{ 'Habilitar el'}} <strong>{{ '`Zonas disponibles`' }}</strong> {{'característica' }}
                    </div>
                    <div class="col-sm-6">
                        <label
