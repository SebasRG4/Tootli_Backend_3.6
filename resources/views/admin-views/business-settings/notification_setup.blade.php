    ...
    <div class="content container-fluid">
        ...
        <div class="page-header d-flex flex-wrap align-items-center justify-content-between">
            ...
            <ul class="nav nav-tabs border-0 nav--tabs nav--pills mb-4">
                <li class="nav-item">
                    <a class="nav-link {{ request()?->type == null || request()?->type == 'admin' ?  'active' : '' }} " href="{{ route('admin.business-settings.notification_setup' ,['type' =>  'admin'])  }}">{{ 'Administración' }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{  request()?->type == 'store' ?  'active' : '' }} " href="{{ route('admin.business-settings.notification_setup' ,['type' =>  'store'])  }}">{{ 'Negocio' }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()?->type == 'customers' ?  'active' : '' }}"   href="{{ route('admin.business-settings.notification_setup' ,['type' =>  'customers'])  }}">{{ 'Clientes' }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()?->type == 'deliveryman' ?  'active' : '' }} "  href="{{ route('admin.business-settings.notification_setup' ,['type' =>  'deliveryman'])  }}">{{ 'repartidor' }}</a>
                </li>
            </ul>
