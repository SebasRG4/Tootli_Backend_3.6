 <div class="modal-body">
     <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-15">
         <div>
             <h3 class="title-clr mb-0">{{ $coupon['title'] }}
                 {{ in_array($coupon['coupon_type'], ['free_delivery']) ? 'Entrega gratuita' : ($coupon['discount_type'] == 'amount' ? '(' . \App\CentralLogics\Helpers::format_currency($coupon['discount']) . ')' : '(' . $coupon['discount'] . '%)') }}
             </h3>
             <div class="d-flex align-items-center gap-1">
                 <span class="fs-14">{{ 'Duración:' }}</span>
         ...
         @if (!in_array($coupon['coupon_type'], ['free_delivery']))
             <div class="bg-warning-10 py-2 px-3 rounded text-center">
                 ...
                 <p class="fs-16 text_FF7500 m-0">{{ 'Descuento' }}</p>
             </div>
         @endif
     </div>
     <!-- <ul class="coupon-details-list d-flex flex-wrap bg-light rounded p-3 mb-3">
         <li class="d-flex flex-sm-nowrap flex-wrap list-none li align-items-center gap-1">
             <span class="fs-14 w-135px d-block min-w-135px">{{ 'tipo de cupón' }} </span>
             <span>:</span>
         ...
         @if ($coupon['coupon_type'] == 'store_wise')
             <li class="d-flex flex-sm-nowrap flex-wrap list-none align-items-center gap-1">
                 <span class="fs-14 w-135px d-block min-w-135px">{{ 'Tienda seleccionada' }} </span>
                 <span>:</span>
                 <span class="fs-14 text-title">{{ $coupon?->store?->name }}</span>
             </li>
         @elseif(count($zoneData) > 0)
             <li class="d-flex flex-sm-nowrap flex-wrap list-none align-items-center gap-1">
                 <span class="fs-14 w-135px d-block min-w-135px">{{ 'Zonas seleccionadas' }} </span>
                 <span>:</span>
         ...
         <li class="d-flex flex-sm-nowrap flex-wrap list-none align-items-center gap-1">
             <span class="fs-14 w-135px d-block min-w-135px">{{ 'Límite para el mismo usuario' }} </span>
             <span>:</span>
             <span class="fs-14 text-title">{{ $coupon['limit'] }}</span>
         </li>
         <li class="d-flex flex-sm-nowrap flex-wrap list-none align-items-center gap-1">
             <span
                 class="fs-14 w-135px d-block min-w-135px">{{ 'Descuento máximo' }}({{ \App\CentralLogics\Helpers::currency_symbol() }})
             </span>
             <span>:</span>
             <span
         ...
         <li class="d-flex flex-sm-nowrap flex-wrap list-none align-items-center gap-1">
             <span
                 class="fs-14 w-135px d-block min-w-135px">{{ 'Compra mínima' }}({{ \App\CentralLogics\Helpers::currency_symbol() }})
             </span>
             <span>:</span>
             <span
         ...
         <li class="d-flex flex-sm-nowrap flex-wrap list-none gap-1">
             <span class="fs-14 w-135px d-block min-w-135px">{{ 'cliente seleccionado' }} </span>
             <span>:</span>
             <span class="fs-14 text-title">
     ...
     <div class="bg-light rounded p-3">
         <h5 class="title-clr mb-15">{{ 'Código de cupón' }}</h5>
