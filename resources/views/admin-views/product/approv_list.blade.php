...
@section('title','Solicitudes de nuevos artículos')
            ...
            <div class="row align-items-center g-2">
                <div class="col-md-9 col-12">
                    <h1 class="page-header-title">
                        ...
                        <span>
                            {{'Solicitudes de nuevos artículos'}} <span class="badge badge-soft-dark ml-2" id="foodCount">{{$items->total()}}</span>
                        </span>
                    </h1>
                </div>
                    ...
                    @if(!isset(auth('admin')->user()->zone_id))
                        <div class="select-item">
                            <select name="zone_id" class="form-control js-select2-custom set-filter"
                                ...
                                <option value="all" {{!request('zone_id')?'selected':''}}>{{ 'Todas las Zonas' }}</option>
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="select-item">
                        <select name="sub_category_id" class="form-control js-select2-custom set-filter" data-placeholder="{{ 'seleccionar subcategoría' }}" id="sub-categories" data-url="{{url()->full()}}" data-filter="sub_category_id">
                            ...
                            <option selected>{{'Sin subcategoría'}}</option>
                            @else
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="select-item">
                        ...
                        data-url="{{url()->full()}}" data-filter="filter">
                            <option {{ !isset($filter)? 'selected' : '' }} >{{ 'Todos los tipos' }}</option>
                            <option value="pending" {{ isset($filter) && $filter == 'pending' ? 'selected' : '' }} >{{ 'Pendiente' }}</option>
                            <option value="rejected" {{ isset($filter) && $filter == 'rejected' ? 'selected' : '' }} >{{ 'rechazado' }}</option>
                            <option value="custom" {{ isset($filter) && $filter == 'custom' ? 'selected' : '' }} >{{ 'Fecha personalizada' }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="search--button-wrapper justify-content-end">
                    ...
                    {{-- @csrf --}}
                        ...
                        <div class="input-group input--group">
                            <input id="datatableSearch" name="search" value="{{ request()?->search ?? null }}" type="search" class="form-control h--40px" placeholder="{{'ej: nombre del elemento de búsqueda'}}" aria-label="{{'buscar aquí'}}">
            ...
            <div class="table-responsive datatable-custom" id="table-div">
                <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <tr>
                        ...
                        <th class="text-title border-0">{{'precio'}}</th>
                        <!-- <th class="text-title border-0">{{'IVA/Impuesto'}}</th> -->
                        <th class="text-title border-0">{{'estado'}}</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    ...
                                    data-original-title="{{ 'Vista' }}" href="{{route('admin.item.requested_item_view',['id'=> $item['id']])}}">
                                    ...
                                    data-url="{{route('admin.item.approved',[ 'id'=> $item['id']])}}" data-message="{{'Quieres aprobar este producto'}}"
                                    ...
                                    @if($item->is_rejected == 0)
                                        ...
                                        data-original-title="{{ 'denegar' }}" data-url="{{ route('admin.item.deny', ['id'=> $item['id']]) }}" data-message="{{ 'quieres negar este producto' }}"
                                        href="javascript:"><i class="tio-clear font-weight-bold"></i></a>
                                    @endif
                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                        href="{{route('admin.item.edit',[$item['id'], 'temp_product' => true])}}" title="{{'editar elemento'}}"><i class="tio-edit"></i>
                                    </a>
