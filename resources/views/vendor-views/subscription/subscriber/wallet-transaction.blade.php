...
@section('title','Transacciones de la tienda')
    ...
    <div class="content container-fluid">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal mb-4">
            <ul class="nav nav-tabs border-0 nav--tabs nav--pills">
                <li class="nav-item">
                    <a href="{{ route('vendor.subscriptionackage.subscriberDetail',$store->id) }}" class="nav-link ">{{ 'Detalles de suscripción' }} </a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('vendor.subscriptionackage.subscriberTransactions',$store->id) }}" class="nav-link">{{ 'Actas' }}</a>
                </li>
                <li class="nav-item">
                    <a href="#" class="nav-link active">{{ 'Reembolsos de suscripción' }}</a>
                </li>
            </ul>
        </div>
                ...
                {{-- <div class="search--button-wrapper justify-content-end">
                    ...
                    <form class="search-form">
                        <div class="input-group input--group">
                            <input name="search" type="search" value="{{ request()?->search }}" class="form-control h--40px" placeholder="{{ 'Ej: buscar por ID de transacción o nombre de tienda' }}" aria-label="Search here">
                ...
                <div class="table-responsive">
                    <table class="table table-borderless middle-align __txt-14px">
                        <thead class="thead-light white--space-false">
                            ...
                            <th class="border-top px-4 border-bottom">{{ 'Nombre del paquete' }}</th>
                            <th class="border-top px-4 border-bottom">{{ 'Monto del reembolso' }}</th>
                            <th class="border-top px-4 border-bottom">{{ 'Reembolsado por' }}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-4">
                                    <div class="w--120px text-title text-right pr-5">{{ str_replace(['validity_left_'], '', $transaction->reference)  }} {{ 'Días' }}</div>
                                </td>
